...
5. 前端轮询 `/api/files/session/:fileId/status`，后端每次调用 DescribeFile 返回状态并更新卡片显示
...
修改接口：
- `POST /api/chat/stream` - 支持 `sessionFileIds` 参数，通过 `rag_options.session_file_ids` 注入到百炼 API

新增函数：
- `createBailianClient()` - 创建百炼 SDK 客户端
- `getBailianClient()` - 获取全局客户端实例（单例模式）
- `applyFileUploadLease()` - 申请上传租约（Step B1）
- `uploadToLeaseUrl()` - 上传到 OSS 预签名 URL（Step B2）
- `addSessionFile()` - 注册会话文件（Step B3）
- `describeFile()` - 查询文件状态（Step B4）
- `waitForFileReady()` - 轮询等待文件就绪（已定义但当前未使用）
...
新增 npm 依赖（已在 package.json 中配置）：
- `multer` (^1.4.5-lts.1) - 处理 multipart/form-data 文件上传
- `form-data` (^4.0.0) - 发送表单数据
...
1. **鉴权区分**:
   - 文件上传使用 RAM AK/SK（BAILIAN_ACCESS_KEY_ID/SECRET）
   - AI 对话使用 DashScope API Key（DASHSCOPE_API_KEY）
...
4. **空间一致性**:
   ...
   - 默认 Workspace ID: `llm-njsp72bv281ofywg`

5. **文件ID格式**:
   - 有效的会话文件ID必须以 `file_session_` 开头
   - 后端会过滤无效的文件ID
