...
文件上传功能必填 (使用 RAM AK/SK):
- `BAILIAN_ACCESS_KEY_ID` - 阿里云 RAM AccessKey ID
- `BAILIAN_ACCESS_KEY_SECRET` - 阿里云 RAM AccessKey Secret

文件上传可选:
- `BAILIAN_WORKSPACE_ID` - 百炼业务空间 ID (未设置时默认 `llm-njsp72bv281ofywg`)
...
- AI 对话功能正常（流式响应）。
- 文件上传功能正常（如已配置 BAILIAN_ACCESS_KEY_ID/SECRET）。
...
### Q: 文件上传返回 CONFIG_ERROR？
A: 确保配置了 `BAILIAN_ACCESS_KEY_ID` 和 `BAILIAN_ACCESS_KEY_SECRET`。
