...
BAILIAN_WORKSPACE_ID=llm-njsp72bv281ofywg  # 业务空间 ID
```
...
| aiType | 描述 | 环境变量 |
|--------|------|---------|
| `abap-clean-core` | ABAP Clean Core 助手 | `DASHSCOPE_APP_ID_ABAP` |
...
// 文件上传处理函数
async function applyFileUploadLease() { /* ... */ }
async function uploadToLeaseUrl() { /* ... */ }
async function addSessionFile() { /* ... */ }
async function describeFile() { /* ... */ }
async function waitForFileReady() { /* ... */ }

// 会话管理
function shouldUseSessionId() { /* ... */ }
...
2. **查看 API 调用详情**:
   - 在 `_callAIStream` 等函数中添加 `console.log`
