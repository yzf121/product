...
> - `BAILIAN_ACCESS_KEY_ID/SECRET` 是 RAM AccessKey，用于文件上传功能
...
**问题**: 文件上传返回 `CONFIG_ERROR`

**解决方案**:
1. 确保配置了 `BAILIAN_ACCESS_KEY_ID` 和 `BAILIAN_ACCESS_KEY_SECRET`（RAM AccessKey，非 DashScope API Key）
2. 确保 AccessKey 有百炼相关权限
3. 确保 `BAILIAN_WORKSPACE_ID` 正确（默认值: `llm-njsp72bv281ofywg`）
4. 文件和应用必须在同一业务空间下

**查看日志**:
```bash
cf logs abap-clean-core-ai-fiori-new-srv --recent | grep "\[FileUpload\]"
```
